...
function create_post_type_news() {
  ...
  $args = [
    ...
    'rewrite' => ["slug" => "site2/news", "with_front" => false],
  ];
...
//----------------------------------------------
function add_post_tag_archive_news( $wp_query ) {
  if ($wp_query->is_main_query() && $wp_query->is_tag()) {
    $wp_query->set( 'post_type', array('post','news'));
  }
}
add_action( 'pre_get_posts', 'add_post_tag_archive_news');

//----------------------------------------------
// カテゴリーアーカイブにて、post_typeに今回のカスタム投稿タイプを追加する
//----------------------------------------------
function add_post_category_archive_news( $wp_query ) {
  if ($wp_query->is_main_query() && $wp_query->is_category()) {
  $wp_query->set( 'post_type', array('post','news'));
  }
}
add_action( 'pre_get_posts', 'add_post_category_archive_news');
...
function url_rewrite_rules_news() {
  add_rewrite_rule('site2/news/category/([^/]+)/page/([0-9]+)/?$', 'index.php?cat_news=$matches[1]&paged=$matches[2]', 'top');
